...
import com.ibcs.idsdp.dpptapp.model.repositories.DppProjectManagementSetupMasterRepository;
import com.ibcs.idsdp.dpptapp.model.repositories.DppProjectManagementSetupRepository;
import com.ibcs.idsdp.dpptapp.web.dto.request.*;
...
public class DppProjectManagementSetupServiceImpl extends BaseService<DppProjectManagementSetup, DppProjectManagementSetupDto> implements DppProjectManagementSetupService {
     ...
     */
    @Override
    public ResponseWithResults getProjectSetupListByProject(String pcUuid) throws IOException {
        ...
        List<DppProjectManagementSetup> managementSetup =
                dppProjectManagementSetupRepository.findAllByProjectConceptUuidAndIsDeletedOrderByIdAsc(pcUuid, false);
        ...
        Optional<DppProjectManagementSetupMaster> dppProjectManagementSetupMaster = dppProjectManagementSetupMasterRepository.findByProjectConceptUuid(pcUuid);
        if (dppProjectManagementSetupMaster.isPresent()) {
            Optional<Attachment> attachment = attachmentRepository.findById(dppProjectManagementSetupMaster.get().getAttachmentId());
            if(attachment.isPresent()){
                File file = ResourceUtils.getFile("src/main/resources/static/files/"+attachment.get().getFileName());
                String image="";
                String extension = FilenameUtils.getExtension(file.getName());
                if (extension.equalsIgnoreCase("pdf") || extension.equalsIgnoreCase("doc") || extension.equalsIgnoreCase("docx")){
                    image = attachment.get().getFileName();
                }else{
