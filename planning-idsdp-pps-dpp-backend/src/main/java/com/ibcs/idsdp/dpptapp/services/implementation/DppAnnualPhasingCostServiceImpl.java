...
public class DppAnnualPhasingCostServiceImpl extends BaseService<DppAnnualPhasingCost, DppAnnualPhasingCostDTO>
	...
	public ResponseEntity<DppAnnualPhasingCostWithChildResponse> saveWithChild(
        ...
        dppAnnualPhasingCostTabDetails(dppAnnualPhasingCostDTO);
	...
	public ResponseEntity<DppAnnualPhasingCostWithChildResponse> updateWithChild(
        ...
        dppAnnualPhasingCostTabDetails(dppAnnualPhasingCostWithChildRequest);
    ...
    private void dppAnnualPhasingCostTabDetails(DppAnnualPhasingCostWithChildRequest dppAnnualPhasingCostWithChildRequest) {
        List<DppAnnualPhasingCostTabDetailsDTO> dppAnnualPhasingCostTabDetails =
                dppAnnualPhasingCostWithChildRequest.getDppAnnualPhasingCostTabDetails().stream()
                        .peek(dppAnnualPhasingCostTabDetailsDTO -> {
                            dppAnnualPhasingCostTabDetailsDTO.setIsBasis(
                                    dppAnnualPhasingCostTabDetailsDTO.getIsBasis() != null && dppAnnualPhasingCostTabDetailsDTO.getIsBasis()
                            );
                            dppAnnualPhasingCostTabDetailsDTO.setIsMajor(
                                    dppAnnualPhasingCostTabDetailsDTO.getIsMajor() != null && dppAnnualPhasingCostTabDetailsDTO.getIsMajor()
                            );
                        })
                        .collect(Collectors.toList());
        dppAnnualPhasingCostWithChildRequest.setDppAnnualPhasingCostTabDetails(dppAnnualPhasingCostTabDetails);
    }
	...
	private List<DppAnnualPhasingCostTotalDTO> createChild(DppAnnualPhasingCostWithChildRequest dppAnnualPhasingCostDTO,
		...
		List<DppFiscalYearDTO> list1 = dppFiscalYearServiceImp.getByProjectConceptIdAndComponentName(
				annalPhasingCost.getProjectConceptId(), dppAnnualPhasings.get(0));
		List<DppFiscalYearDTO> list2 = dppFiscalYearServiceImp.getByProjectConceptIdAndComponentName(
				annalPhasingCost.getProjectConceptId(), dppAnnualPhasings.get(1));
	...
	private void deleteWhichFiscalYearIsNotExist(DppAnnualPhasingCostWithChildRequest dppAnnualPhasingCostDTO,
		...
		Map<String, List<DppFiscalYearDTO>> listMap1 = dppFiscalYearServiceImp
				.getByProjectConceptIdAndComponentName(annalPhasingCost.getProjectConceptId(), dppAnnualPhasings.get(0))
		...
		Map<String, List<DppFiscalYearDTO>> listMap2 = dppFiscalYearServiceImp
				.getByProjectConceptIdAndComponentName(annalPhasingCost.getProjectConceptId(), dppAnnualPhasings.get(1))
				.stream().collect(groupingBy(DppFiscalYearDTO::getFiscalYear));
	...
	private void validationCheck(DppAnnualPhasingCostWithChildRequest annalPhasingCost,
		...
		if (annalPhasingCost.getComponentName().equals(DppAnnualPhasing.Revenue_Component)
            ...
            if (e.getIsBasis() == null) {
                throw new ServiceExceptionHolder.InvalidRequestException("Is Basis can't be null");
            }
	...
	public ResponseEntity<DppAnnualPhasingCostWithChildResponse> getByProjectConceptIdAndComponentName(
		...
		Optional<DppAnnualPhasingCost> dppAnnualPhasingOfCost = repository
...
//				fiscalYearWiseTotal.setQty(fiscalYearWiseTotal.getQty() +fyd.getQty());
			...
			if (e.getEconomicCodeId() != null){
				EconomicCode economicCode = configurationClientService.getByEconomicCodeId(e.getEconomicCodeId());
				e.setEconomicCodeDTO(economicCode);
			}
			if (e.getSubEconomicCodeId() != null){
				SubEconomicCode subEconomicCode = configurationClientService.getBySubEconomicCodeId(e.getSubEconomicCodeId());
				e.setSubEconomicCodeDTO(subEconomicCode);
			}
			if (e.getSubEconomicCodeId() != null){
				UnitType unitType = configurationClientService.getByUnitTypeId(e.getUnitId());
	 ...
	 */
	@Override
	public ResponseEntity<List<GrandTotalResponse>> getGrandTotalByProjectConceptId(Long projectConceptId) {
	...
	private void setRevenueAndCapitalAndContingencyAndGrandTotal(Long projectConceptId,
		...
		if (dppAnnualPhasingCostTotalRevenue != null) {
			...
			Optional<GrandTotalResponse> revenue = response.stream().filter(res -> res.getDppAnnualPhasing().toString().equals("Revenue_Component")).findAny();
			if (revenue.isPresent()) revenue.get().setDppAnnualPhasingCostTotal(Collections.singletonList(finalDppAnnualPhasingCostTotal));
		}
		...
		if (dppAnnualPhasingCostTotalCapital != null) {
			DppAnnualPhasingCostTotalDTO finalDppAnnualPhasingCostTotal = new ModelMapper()
			...
			Optional<GrandTotalResponse> capital = response.stream().filter(res -> res.getDppAnnualPhasing().toString().equals("Capital_Component")).findAny();
			if (capital.isPresent()) capital.get().setDppAnnualPhasingCostTotal(Collections.singletonList(finalDppAnnualPhasingCostTotal));
		}
		...
		if (annualContingencyPhasingCost.isPresent()) {
			...
			Optional<GrandTotalResponse> contingency = response.stream().filter(res -> res.getDppAnnualPhasing().toString().equals("Contingency")).findAny();
			if (contingency.isPresent()) contingency.get().setDppAnnualPhasingCostTotal(contingencyList);
		}
	...
	public ResponseEntity<List<YearWisePhysicalAndFinancialTarget>> getYearWisePhysicalAndFinancialTargetByProjectConceptId(
		...
		List<DppAnnualPhasingCostDTO> dppAnnualPhasingCostDTOS = convertForRead(
				repository.findByProjectConceptIdAndIsDeleted(conceptId, false));

		if (dppAnnualPhasingCostDTOS.isEmpty())
			return new ResponseEntity<>(null, HttpStatus.OK);

		Optional<DppObjectiveCost> dppObjectiveCost = dppObjectiveCostRepository
				.findByProjectConceptUuidAndIsDeleted(dppAnnualPhasingCostDTOS.get(0).getProjectConceptUuid(), false);
...
//                    dppAnnualPhasingCostDetails.setWeight((dapc.getTotalAmount() > 0) ? (dapc.getTotalAmount() / (total.get() * 104)) : 0);
			...
			for (int i = 0; i < details.size(); i++) {
				List<FinancialYears> yearList = new ArrayList<>();
				for (int j = 0; j < details.get(i).getYears().size(); j++) {
					FinancialYears financialYear = details.get(i).getYears().get(j);
					int finalJ = j;
					Double totalFA = details.stream().collect(Collectors.summingDouble(yr -> yr.getYears().get(finalJ).getFinancialAmount()));
					Double totalPA = details.stream().collect(Collectors.summingDouble(yr -> yr.getYears().get(finalJ).getPercentageOfItem()));
					Double totalPP = details.stream().collect(Collectors.summingDouble(yr -> yr.getYears().get(finalJ).getPercentageOfProject()));
					financialYear.setTotalFinancialAmount(totalFA);
					financialYear.setTotalPercentageOfItem(totalPA);
					financialYear.setTotalPercentageOfProject(totalPP);
					yearList.add(financialYear);
				}
				details.get(i).getYears().clear();
				details.get(i).setYears(yearList);
			}
