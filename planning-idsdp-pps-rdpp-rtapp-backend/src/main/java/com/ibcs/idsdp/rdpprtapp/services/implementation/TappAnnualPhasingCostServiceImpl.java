...
import com.ibcs.idsdp.rdpprtapp.services.TappAnnualPhasingCostService;
...
public class TappAnnualPhasingCostServiceImpl extends BaseService<TappAnnualPhasingCost, TappAnnualPhasingCostDTO>
	...
	protected TappAnnualPhasingCostServiceImpl(TappAnnualPhasingCostRepository repository,
			...
			PpsDppTappClientService ppsDppTappClientService, TappObjectiveCostRepository tappObjectiveCostRepository
	) {
		super(repository);
	...
	public ResponseEntity<TappAnnualPhasingCostWithChildResponse> saveWithChild(
		...
		return getByProjectConceptIdAndComponentName(new ProjectConceptIdAndComponentNameRequest() {
			{
				setRtappMasterId(response.getRtappMasterId());
	...
	public ResponseEntity<TappAnnualPhasingCostWithChildResponse> getByProjectConceptIdAndComponentName(
		...
		Optional<TappAnnualPhasingCost> tappAnnualPhasingOfCost = repository
				.findByRtappMasterIdAndComponentNameAndIsDeleted(request.getRtappMasterId(),
	 ...
	 */
	@Override
	public ResponseEntity<List<GrandTotalResponseTapp>> getGrandTotalByProjectConceptId(Long rtappMasterId) {
		...
		List<TappAnnualPhasingCostTotalDTO> revenuesTotal = tappAnnualPhasingCostTotalServiceImp
				.getByProjectConceptIdAndComponentName(rtappMasterId, DppAnnualPhasing.Revenue_Component);
		List<TappAnnualPhasingCostTotalDTO> capitalTotal = tappAnnualPhasingCostTotalServiceImp
				.getByProjectConceptIdAndComponentName(rtappMasterId, DppAnnualPhasing.Capital_Component);
		List<TappFiscalYear> contingency = tappFiscalYearRepository
				.getByProjectConceptIdAndComponentName(rtappMasterId, DppAnnualPhasing.Contingency.ordinal());
		...
		setRevenueAndCapitalAndContingencyAndGrandTotal(rtappMasterId, response);
	 ...
	 * @param response
	 */
	private void setRevenueAndCapitalAndContingencyAndGrandTotal(Long rtappMasterId,
			List<GrandTotalResponseTapp> response) {
		Optional<TappAnnualPhasingCost> annualRevenuePhasingCost = repository
				.findByRtappMasterIdAndComponentNameAndIsDeleted(rtappMasterId,
		...
		Optional<TappAnnualPhasingCost> annualContingencyPhasingCost = repository
				.findByRtappMasterIdAndComponentNameAndIsDeleted(rtappMasterId, DppAnnualPhasing.Contingency,
						false);
	...
	private void setAllGrandTotal(List<GrandTotalResponseTapp> response) {
		Optional<GrandTotalResponseTapp> grandTotalItem = response.stream().filter(f -> f.getComponentName().equals(DppAnnualPhasing.Grand_Total)).findAny();
		if (grandTotalItem.isPresent()) {
			TappAnnualPhasingCostTotalDTO grandTotal = grandTotalItem.get().getTappAnnualPhasingCostTotal().get(0);
			List<TappAnnualCostTotalWithFiscalYear> yearWiseTotalList = grandTotalItem.get().getGrandTotal();

			TappAnnualPhasingCostTotalDTO total = new TappAnnualPhasingCostTotalDTO();
			total.setTotalAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getTotalAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getTotalAmount());
			total.setGobAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getGobAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getGobAmount());
			total.setGobFeAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getGobFeAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getGobFeAmount());
			total.setGobThruAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getGobThruAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getGobThruAmount());
			total.setSpAcAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getSpAcAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getSpAcAmount());
			total.setThruPdAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getThruPdAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getThruPdAmount());
			total.setThruDpAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getThruDpAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getThruDpAmount());
			total.setOwnFundAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getOwnFundAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getOwnFundAmount());
			total.setOwnFundFeAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getOwnFundFeAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getOwnFundFeAmount());
			total.setOtherAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getOtherAmount())
					.mapToDouble(Double::doubleValue).sum() + grandTotal.getOtherAmount());
			total.setOtherFeAmount(yearWiseTotalList.stream().map(fs -> fs.getTappAnnualPhasingCostTotal().getOtherFeAmount())
...
////        }
	...
	public DetailsEstimatedCostResponse getDetailsEstimatedCost(Long id) {
		try {
			...
			List<TappAnnualPhasingCost> tappAnnualPhasingCostList = repository
					.findAllByRtappMasterIdAndIsDeleted(id, false);
	...
	public DetailsEstimatedCostResponse calculationEstimatedCostAnnexureVaData(
		...
		for (EstimatedCostAnnexureDTO estimatedCostAnnexureDTO : detailsEstimatedCostResponse
			...
			for (EstimatedCostTabDetailsDTO estimatedCostTabDetailsDTO : estimatedCostAnnexureDTO
				...
				Double totalProjectConstCal = (100 / summationOfAllCost * 104
						* estimatedCostTabDetailsDTO.getTotalCostCalculated()) / 100;
	...
	/**
	 * Get Grand Total By RTAPP Master Id
	 *
	 * @param rtappMasterId
	 * @return
	 */
	@Override
	public ResponseEntity<TappGrandTotalDifferenceResponse> getGrandTotalByMasterId(Long rtappMasterId) {
		TappGrandTotalDifferenceResponse grandTotalDifferenceResponse = new TappGrandTotalDifferenceResponse();
		...
		Optional<TappObjectiveCost> tappObjectiveCost = tappObjectiveCostRepository.findById(rtappMasterId);
		ResponseEntity<List<GrandTotalResponseTapp>> refGrandTotalRes = null;
		if(tappObjectiveCost.isPresent()){
			if(tappObjectiveCost.get().getRevisedVersion().equals("1st Revised")){
				refGrandTotalRes = ppsDppTappClientService.getTappGrandTotalByProjectConceptId(tappObjectiveCost.get().getProjectConceptMasterId());
				grandTotalDifferenceResponse.setReferenceGrandTotal(refGrandTotalRes.getBody());

				projectConceptIdAndComponentNameRequest.setProjectConceptId(tappObjectiveCost.get().getProjectConceptMasterId());
				projectConceptIdAndComponentNameRequest.setComponentName("Revenue_Component");
				ResponseEntity<TappAnnualPhasingCostWithChildResponse> itemWiseRevenueRes = ppsDppTappClientService.getTappByProjectConceptIdAndComponentName(projectConceptIdAndComponentNameRequest);
				...
				ResponseEntity<TappAnnualPhasingCostWithChildResponse> itemWiseCapitalRes = ppsDppTappClientService.getTappByProjectConceptIdAndComponentName(projectConceptIdAndComponentNameRequest);
				grandTotalDifferenceResponse.setReferenceItemWiseCapital(itemWiseCapitalRes.getBody());

			}else {
				refGrandTotalRes = getGrandTotalByProjectConceptId(tappObjectiveCost.get().getReferenceId());
				grandTotalDifferenceResponse.setReferenceGrandTotal(refGrandTotalRes.getBody());

				projectConceptIdAndComponentNameRequest.setRdppMasterId(tappObjectiveCost.get().getReferenceId());
				projectConceptIdAndComponentNameRequest.setComponentName("Revenue_Component");
				ResponseEntity<TappAnnualPhasingCostWithChildResponse> itemWiseRevenueRes = getByProjectConceptIdAndComponentName(projectConceptIdAndComponentNameRequest);
				grandTotalDifferenceResponse.setReferenceItemWiseRevenue(itemWiseRevenueRes.getBody());

				projectConceptIdAndComponentNameRequest.setComponentName("Capital_Component");
				ResponseEntity<TappAnnualPhasingCostWithChildResponse> itemWiseCapitalRes = getByProjectConceptIdAndComponentName(projectConceptIdAndComponentNameRequest);
				grandTotalDifferenceResponse.setReferenceItemWiseCapital(itemWiseCapitalRes.getBody());
			}
		}

		ResponseEntity<List<GrandTotalResponseTapp>> currentGrandTotaRes = getGrandTotalByProjectConceptId(rtappMasterId);
		grandTotalDifferenceResponse.setCurrentGrandTotal(currentGrandTotaRes.getBody());

		projectConceptIdAndComponentNameRequest.setRtappMasterId(rtappMasterId);
		projectConceptIdAndComponentNameRequest.setComponentName("Revenue_Component");
		ResponseEntity<TappAnnualPhasingCostWithChildResponse> itemWiseRevenueRes = getByProjectConceptIdAndComponentName(projectConceptIdAndComponentNameRequest);
		grandTotalDifferenceResponse.setCurrentItemWiseRevenue(itemWiseRevenueRes.getBody());

		projectConceptIdAndComponentNameRequest.setComponentName("Capital_Component");
		ResponseEntity<TappAnnualPhasingCostWithChildResponse> itemWiseCapitalsRes = getByProjectConceptIdAndComponentName(projectConceptIdAndComponentNameRequest);
	...
	@Override
	public ResponseEntity<List<GrandTotalResponseTapp>> getYearWiseEstimatedCost(String pcUuid){

		/** (Estimated Cost of the project calculation) */
		List<GrandTotalResponseTapp> estimatedCosts = new ArrayList<>();

		List<TappObjectiveCost> objList = tappObjectiveCostRepository.findAllByProjectConceptUuidAndIsDeletedOrderByIdDesc(pcUuid, false);

		/** Annual Phasing cost Previous revised Data get here */
		for (int i = 0; i < objList.size(); i++) {
			TappObjectiveCost val = objList.get(i);
			ResponseEntity<List<GrandTotalResponseTapp>>  grandTotal2 = getGrandTotalByProjectConceptId(val.getId());
			GrandTotalResponseTapp gtDto = grandTotalCal(grandTotal2);
			gtDto.setRevisedVersion(objList.get(i).getRevisedVersion());
			estimatedCosts.add(gtDto);
		}

		/** Dpp Grand Total or original version*/
		ResponseEntity<List<GrandTotalResponseTapp>> dppGrandTotal = ppsDppTappClientService.getTappGrandTotalByProjectConceptId(objList.get(0).getProjectConceptMasterId());
		GrandTotalResponseTapp a = grandTotalCal(dppGrandTotal);
	...
	private GrandTotalResponseTapp grandTotalCal(ResponseEntity<List<GrandTotalResponseTapp>> grandTotal) {
		...
		return new GrandTotalResponseTapp();
	}
