...
import com.ibcs.idsdp.rdpprtapp.model.repositories.LogFrameRepository;
...
public class LogFrameServiceImp extends BaseService<DppLogFrame, LogFrameDTO> implements LogFrameService {
    ...
    protected DppLogFrame convertForCreate(LogFrameDTO logFrameDTO) {
        ...
        dppLogFrame.setRdppMasterId(logFrameDTO.getRdppMasterId());
    ...
    public ResponseWithResults getLogFrame(String pcUuid) {
        try {
            ...
            if (!logFrame.isPresent()){
                ...
                response.setInputNS(dppLogFrame.getInputIA());
                ...
                response.setOutputNS(dppLogFrame.getOutputIA());
                ...
                response.setObjectiveNS(dppLogFrame.getObjectiveIA());
    ...
    /* -------------  for Get Log Frame Data  ---------------*/
    @Override
    public ResponseWithResults getLogFrameById(Long id) {
        try {
            Optional<DppLogFrame> logFrame = logFrameRepository.findByRdppMasterId(id);
            if (!logFrame.isPresent()){
                return new ResponseWithResults(0, "Not Found","");
            }else{
                DppLogFrame dppLogFrame = logFrame.get();
    ...
    @Override
    public LogFrameResponse updateLogFrame(LogFrameResponse response, Long id) {
        Optional<DppLogFrame> optionalDppLogFrame = logFrameRepository.findByRdppMasterId(id);
        if(!optionalDppLogFrame.isPresent()){
            ...
            logFrame.setInputNS(response.getInputIA());
            ...
            logFrame.setOutputNS(response.getOutputIA());
            ...
            logFrame.setObjectiveNS(response.getObjectiveIA());
