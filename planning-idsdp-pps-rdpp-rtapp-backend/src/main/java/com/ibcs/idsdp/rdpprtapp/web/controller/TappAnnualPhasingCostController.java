...
import com.ibcs.idsdp.rdpprtapp.constants.TappAnnualPhasingCostConstant;
...
//    }
    ...
    @GetMapping("get-grand-total-by-master-id/{rtappMasterId}")
    public ResponseEntity<TappGrandTotalDifferenceResponse> getGrandTotalByMasterId(@PathVariable Long rtappMasterId) {
        return service.getGrandTotalByMasterId(rtappMasterId);
    }

    @GetMapping("get-year-wise-estimated-cost-by/{pcUuid}")
    public ResponseEntity<List<GrandTotalResponseTapp>> getYearWiseEstimatedCost(@PathVariable String pcUuid){
