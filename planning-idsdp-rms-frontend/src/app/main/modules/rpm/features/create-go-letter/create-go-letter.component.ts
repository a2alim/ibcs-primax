...
export class CreateGoLetterComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.getElement.mode == "Edit") {
            ...
            this.frmGroup.controls["approvedStatus"].setValue(this.element.approvedStatus);
            this.frmGroup.controls["enothiNumber"].setValue(this.element.enothiNumber);
            this.frmGroup.controls["bnDate"].setValue(this.element.bnDate);
            this.frmGroup.controls["enDate"].setValue(this.element.enDate);
    ...
    public onSubmit() {
        if (this.formTitle == 'Save') {
            ...
            this.submitData = {
                ...
                "goCode": this.element.goCode,
                "researcherProposalId": this.getInstallmentProcess.m1ResearcherProposalId.id,
                "installmentProcessId": this.getInstallmentProcess.id,
                "installmentTypeId": this.getInstallmentProcess.stInstallmentTypeId,
                "fiscalYearId": this.getInstallmentProcess.m1ResearcherProposalId.stFiscalYearId,
                "researchCatTypeId": this.getInstallmentProcess.researchCategoryType.id,
                "totalAmount": this.getInstallmentProcess.tkAmount,
                "subject": this.frmGroup.value.subject,
                "mailBody": this.frmGroup.value.mailData,
                "templateTypeId": this.templateId ? this.templateId : this.getElement.element.templateTypeId,
                "predefinedTemplateId": this.predifinedTemplateId ? this.predifinedTemplateId : this.getElement.element.predefineTemplate.id,
                "isSend": this.frmGroup.value.isSend,
                "approvedStatus": this.frmGroup.value.approvedStatus,
                "enothiNumber": this.frmGroup.value.enothiNumber,
                "bnDate": this.frmGroup.value.bnDate,
        ...
        if (this.formTitle == 'Save') {
            this.spinner = true;
            this._createGoLetterServiceService.saveData(this.submitData).subscribe(
                res => {
                    if (res.success) {
    ...
    getInstallmentProcessById() {
        if (this.formTitle == 'Save') {
            console.log()
            this._createGoLetterServiceService.getInstallmentProcessById(1).subscribe(res => {
    ...
    selectPredefinedTemplate(event) {
        ...
        // for (let i = 0; i < this.predifinedTemplate.length; i++) {
        //     if (event.id == this.predifinedTemplate[i].id) {
        //         const values = Object.keys(event).map(key => event[key]);
        //         const commaJoinedValues = values.join(",");
        //         const data = commaJoinedValues.split(',');
        //         this.ckeditor.instance.setData(data[4]);
        //     }else{
        //         this.ckeditor.instance.setData('');
        //     }
        //}
