...
export class XXXResearcherProposalComponent extends UnsubscribeAdapterComponent implements OnInit {
   ...
   // @Input() inputDivisionList : any[]
    // @Input() storeDistrictListVal : any[]
    // @Input() storeUpazilaList : any[]

    // @Input() set inputDivisionList(value: any) {
    //     this.storeDivisionList = value;
    //     console.log('this.storeDivisionList nnnn', this.storeDivisionList);
    // }
    // @Input() set inputDistrictList(value: any) {
    //     this.storeDistrictList = value;
    // }
    // @Input() set inputUpazilaList(value: any) {
    //     this.storeUpazilaList = value;
    // }
    ...
    async ngOnInit() {
        ...
        //let a = await this.getDivDisList(1);

        await this.getDivDisList(() => {
            console.log("DO something in callBackFunction();")
        } );
        
        console.log('this.storeDivisionList33 = ', this.storeDivisionList33)
        console.log('ddddddddddd');
        this.proposalInfo.stResearchCatTypeId = Number(this.categoryTypeId);
        await this.onChangrResearchCategory({ value: this.proposalInfo.stResearchCatTypeId });
        ...
        if (this.researcherProfileUuId) {
            await this.getResearcherProfileByUuId();
        }

        console.log('111111 1 storeDivisionList', this.storeDivisionList);
        console.log('researcherProfileUuId 1111', this.researcherProfileUuId);
    ...
    getData() {
        console.log('eeeeeeeeeeee = ', this.num++);
        
        console.log('storeDistrictList eeeeeeeeeeee', this.storeDistrictList);
        console.log('storeDistrictList eeeeeeeeeeee typeof', typeof this.storeDistrictList);
        ...
        if (this.existingProposalInfo.id) {
            ...
            if (divisionIds != null && divisionIds != "") {
                ...
                this.getDistrictByChangeDivision(divisionIds,0,'');
    ...
    setFieldData(objData) {
        ...
        if (objData.upzilaId.length > 0) {
            ...
            this.proposalInfo.upzilaId = JSON.parse(objData.upzilaId);
        }
    }
        ...
        this.researchProfileMultiFormService.getResearcherProfileByUuid(this.researcherProfileUuId).subscribe(
            response => {
                if (response.success) {
                    ...
                    this.getDistrictByChangeDivision(response.obj.divisionId,0,'');
                }
                this.spinner = false;
            },
            error => {
    ...
    getDistrictByChangeDivision(divisionIdList, fromHtml = 0, callBack) {
        ...
        console.log('divisionIdList getDistrictByChangeDivision = ', divisionIdList)

        console.log('$this.storeDivisionList length xxx  typeof = ', typeof this.storeDivisionList);
        ...
        if (Array.isArray(divisionIdList)) {
            $this.proposalInfo.divisionId = divisionIdList;
            
            $this.districtList = []
            divisionIdList.forEach(function(divisionId){

                $this.storeDivisionList.forEach(function (d: any) {
                    if (d.id == divisionId) {
                        $this.proposalInfo.divisionName += d.en_name + ", ";
                    }
                })

                console.log('$this.storeDivisionList length = ', this.storeDivisionList.length);
                $this.storeDistrictList.forEach(function(res){
                    if(res.division_id == divisionId){
        ...
        $this.proposalInfo.divisionName = ($this.proposalInfo.divisionName != "") ? this.removeLastComma($this.proposalInfo.divisionName) : "";

    }

    getDistrictByChangeDivisionX(divisionIdList, fromHtml = 0, callBack) {

        divisionIdList = (typeof divisionIdList == 'string') ? JSON.parse(divisionIdList) : divisionIdList;
        let oldDistrictIdsVal = this.proposalInfo.districtId;
        if (fromHtml == 1) {
            var oldDistrictIds = (typeof oldDistrictIdsVal == 'object') ? oldDistrictIdsVal : (typeof oldDistrictIdsVal == 'string') ? oldDistrictIdsVal : JSON.parse(oldDistrictIdsVal);
        }
        else {
        ...
        // console.log('oldDistrictIds =', oldDistrictIds);
        // return;
        ...
        this.DividionDU.getDistrictsList().subscribe(disList => {
            ...
            if (Array.isArray(divisionIdList)) {
                divisionIdList.forEach((function (divisionId) {
                    ...
                    //Save division names create for easy report
                    $this.storeDistrictList.forEach(function (d: any) {
                        if (d.id == divisionId) {
                    ...
                    disList.forEach(function (disInfo: any) {
                        if (disInfo.division_id == divisionId) {
                            storeDistrictList.push(disInfo);
                            ...
                            if (oldDistrictIds.length > 0) {
                                ...
                                if (Array.isArray(oldDistrictIds)) {
                                    ...
                                    setTimeout(function () {
                                        if (oldDistrictIds.length > 0) {
                                            $this.getUpazilaByChangeDistrict(updateDistrictIds)
                                        }
                                    }, 3000)
                ...
                this.districtList = storeDistrictList;
                if (typeof callBack == "function") {
                    callBack(true);
                }
    ...
    getUpazilaByChangeDistrict(districtsIdList, fromHtml = 0) {
        ...
        districtsIdList = (typeof districtsIdList == 'string') ? JSON.parse(districtsIdList) : districtsIdList;
        let $this = this;

        if (Array.isArray(districtsIdList)) {
            $this.upazilaList = [];
            $this.proposalInfo.districtName = '';
            ...
            districtsIdList.forEach(function(districtId){
                ...
                $this.districtList.forEach(function (d: any) {
                    if (d.id == districtId) {
                        $this.proposalInfo.districtName += d.en_name + ", ";
                    }
                })

                $this.storeUpazilaList.forEach(function(res){
                    if(res.district_id == districtId){
                        $this.upazilaList.push(res);
                    }
                })
            })
        }
        ...
        console.log('$this.proposalInfo.districtName1 =>', $this.proposalInfo.districtName);

        $this.proposalInfo.districtName = ($this.proposalInfo.districtName != "") ? this.removeLastComma($this.proposalInfo.districtName) : "";
        console.log('$this.proposalInfo.districtName2 =>', $this.proposalInfo.districtName);
    }

    getUpazilaByChangeDistrictX(districtsIdList, fromHtml = 0) {
        ...
        if (fromHtml == 1) {
            var oldUpazilaIds = (typeof upzilaId == 'object') ? upzilaId : (typeof upzilaId == 'string') ? upzilaId : JSON.parse(upzilaId);
        } else {
        ...
        if (districtsIdList.length > 0) {
            ...
            this.upazilaList = storeUpazilas;
            //})
        }
        else {
            this.proposalInfo.districtName = "";
    ...
    getDivDisList(callBack){
       
        let $back = 0;
        this.DividionDU.getDivision().subscribe(data => {
            $back++;
            console.log('XXXX = ', this.num++);
            this.storeDivisionList33 = data;
        })
