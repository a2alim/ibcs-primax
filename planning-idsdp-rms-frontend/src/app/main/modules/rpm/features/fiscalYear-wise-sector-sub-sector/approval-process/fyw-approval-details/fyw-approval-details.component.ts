...
@Component({
  selector: 'app-fyw-approval-details',
  templateUrl: './fyw-approval-details.component.html',
  styleUrls: ['./fyw-approval-details.component.scss']
})
export class FywApprovalDetailsComponent implements OnInit {
...
showModal = false;
  ...
  confirm(){
    let doptorToken = sessionStorage.getItem('doptor_token');

    if(!doptorToken){
      this.toastr.error("Doptor Token is Empty!", "", this.config); 
      return;
    }

    this.approvalDataModel.doptor_token = doptorToken;
    ...
    this.enothiApprovalModel.doptorToken = doptorToken;
