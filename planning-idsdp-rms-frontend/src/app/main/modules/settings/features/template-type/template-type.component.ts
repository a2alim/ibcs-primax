...
export class TemplateTypeComponent implements OnInit {
  ...
  displayedColumns: string[] = ['position', 'templateType', 'active', 'action'];
  dataSource: any;
  ...
  getListData() {
    this.templatetypeservice.getListData(this.page, this.pageSize).subscribe(res => {
  ...
  delete(rowUuid) {
    this.templatetypeservice.delete(rowUuid).subscribe(res => {
      if (res.success) {
  ...
  submitForm() {
    ...
    if (this.formTitle == 'Edit') {
      this.templatetypeservice.updateData(this.frmGroup.value).subscribe(
        res => {
          if (res.success) {
            this.spinner = false;
            this.toastr.success(res.message, "Success!", this.config);
      ...
      this.templatetypeservice.saveData(this.frmGroup.value).subscribe(
        res => {
          if (res.success) {
            this.spinner = false;
            this.toastr.success(res.message, "Success!", this.config);
