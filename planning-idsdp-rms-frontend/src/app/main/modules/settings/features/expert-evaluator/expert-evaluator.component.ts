...
@Component({
    selector: 'app-expert-evaluator',
    templateUrl: './expert-evaluator.component.html',
    styleUrls: ['./expert-evaluator.component.scss']
})
export class ExpertEvaluatorComponent implements OnInit {
    // Common Properties
    ...
    constructor(private _fuseTranslationLoaderService: FuseTranslationLoaderService,
        ...
        this.isEvaluator = this.dataService.isEvaluator;
        if (this.isEvaluator) {
            sessionStorage.setItem('isEvaluator', String(this.isEvaluator));
        } else {
            if (sessionStorage.getItem('isEvaluator') == 'true') {
    ...
    ngOnInit(): void {
        ...
        this.evaluatorData=this.dataService.evaluatorData;
        if(this.mode==='edit'){
            this.editRow();
        }
        this.initialize();
    ...
    private addCertificateRow() {
        ...
        // this.certificateDataSourceRows.push(this.expertEvaluatorEducation)
        // this.certificateDataSource = new MatTableDataSource(this.certificateDataSourceRows);
        // this.expertEvaluator.educations.push(new ExpertEvaluatorEducation());

        console.log("certificateDataSource = ", this.certificateDataSource);
    }

    private removeCertificateRow(index) {
        this.certificateDataSourceRows.splice(index, 1);
        this.certificateDataSource = new MatTableDataSource(this.certificateDataSourceRows);
        if(this.certificateDataSourceRows.length == 0){
            this.addCertificateRow();
           // this.expertEvaluator.educations.push(new ExpertEvaluatorEducation());
        }
    ...
    private removeSectorRow(index, id) {
        this.sectorDataSourceRows.splice(index, 1);
        const indexToDelete = this.expertEvaluator.sectorSubSectors.findIndex(f => f.id === id);
        this.expertEvaluator.sectorSubSectors.splice(indexToDelete, 1)
        this.sectorDataSource = new MatTableDataSource(this.sectorDataSourceRows);
    ...
    dateChange(){
        if (this.expertEvaluator.dateOfBirth) {
            const convertAge = new Date(this.expertEvaluator.dateOfBirth);

            if (new Date(convertAge).getTime() > this.todayDate.getTime()) {
                this.expertEvaluator.dateOfBirth = null;
                this.toastr.error('Please enter previous date, this.config');
            }
        }
    }
    initialize() {
    ...
    onSubmit(action, myForm: NgForm) {
        if (myForm.valid) {
            this.spinner = true;
            this.expertEvaluator.educations = this.certificateDataSource?.data || []; // this.certificateDataSourceRows;
            console.log('this.certificateDataSourceRows = ', this.certificateDataSourceRows);
            console.log('this.expertEvaluator.educations ==', this.expertEvaluator.educations);
            console.log("expertEvaluator = ", this.expertEvaluator);
    ...
    naviagetTo() {
        if (this.isEvaluator != true) {
            this.route.navigate(['/expert-evaluator'])
        } else if (this.isEvaluator == true) {
            this.route.navigate(['/evaluator-profile'])
        }
    ...
    editRow() {
        this.expertEvaluatorService.getById(this.evaluatorData.id).subscribe(res => {
            this.expertEvaluator = new ExpertEvaluator();
            this.formTitle = "Edit";
            ...
            if(this.expertEvaluator?.educations.length > 0){
                this.certificateDataSource = new MatTableDataSource(this.certificateDataSourceRows);
            }
            // else{
            //     this.certificateDataSource.push(new MatTableDataSource);// = new MatTableDataSource(this.certificateDataSourceRows);
            // }
            ...
            this.expertEvaluator.sectorSubSectors.forEach(element => {
                ...
                this.sectorSubSector.id = element.id;
