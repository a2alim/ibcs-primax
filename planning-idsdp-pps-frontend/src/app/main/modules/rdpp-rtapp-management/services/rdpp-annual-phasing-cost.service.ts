...
export class RdppAnnualPhasingCostService extends CrudRequestService<IDppAnnualPhasingCost> {
    ...
    private BASE_API_URL_RDPP: string = DppAnnualPhasingCostConstant.RDPP_ANNUAL_PHASING_COST;

    constructor(private http: HttpClient) {
        super(http, DppAnnualPhasingCostConstant.RDPP_ANNUAL_PHASING_COST)
    }
    ...
    getByProjectConceptIdForGettingFiscalYear(conceptId: number): Observable<{ fiscalYear: string }[]> {
        return this.http.get<{ fiscalYear: string }[]>(this.BASE_API_URL_RDPP + DppAnnualPhasingCostConstant.GET_BY_PROJECT_CONCEPT_ID_FOR_CHECKING_FISCAL_YEAR + '/' + conceptId);
    }

    getGrandTotalByProjectConceptId(conceptId: number): Observable<{ dppAnnualPhasing: string, dppAnnualPhasingCostTotal: IDppPhasingCostTotal[], grandTotal: { fiscalYear: string, dppAnnualPhasingCostTotal: IDppPhasingCostTotal } [], allGrandTotal: IDppPhasingCostTotal }[]> {
        return this.http.get<{ dppAnnualPhasing: string, dppAnnualPhasingCostTotal: IDppPhasingCostTotal[], grandTotal: { fiscalYear: string, dppAnnualPhasingCostTotal: IDppPhasingCostTotal } [], allGrandTotal: IDppPhasingCostTotal }[]>(this._BASE_URL + DppAnnualPhasingCostConstant.GET_GRAND_TOTAL_BY_PROJECT_CONCEPT_ID + "/" + conceptId);
    }
    ...
    getByProjectConceptUuid(pcUuid: string) {
        const url: string = environment.ibcs.ppsRdppRtappBackendPoint + DppAnnualPhasingCostConstant.GET_ALL_BY_PCUUID + '/' + pcUuid;
    ...
    getAnnualPhasingCostByPCUuidAndComponentType(pcUuid: string, type: string) {
        const url: string = environment.ibcs.ppsRdppRtappBackendPoint + DppAnnualPhasingCostConstant.GET_ANNUAL_PHASING_COST_BY_PCUUID_AND_TYPE + '/' + pcUuid + '/' + type;
        return this.http.get(url).pipe(map((res: any) => {
            return res;
        }));
    }
    ...
    public getDetailsEstimatedCostInRdpp(conceptId: string): Observable<any> {
        return this.http.get(this.BASE_API_URL_RDPP + DppAnnualPhasingCostConstant.GET_DETAILS_ESTIMATED_COST + '/' + conceptId);
    }
