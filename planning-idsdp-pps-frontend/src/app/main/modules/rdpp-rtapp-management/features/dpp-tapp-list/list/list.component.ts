...
export class ListComponent extends UnsubscribeAdapterComponent implements OnInit {
    ...
    private getProjectTypes() {
            this.projectTypeService.getList().subscribe(res => {
                res.forEach(value => {
                    ...
                    console.log(value.id+"--"+this.projectTypeId)
                });
    ...
    loadProjectSummery() {
        this._router.navigate([`rdpp-rtapp/add-new/${this.conceptId}`]);
    }

    getAmortizationScheduleTotal() {
        this._router.navigate([`rdpp-rtapp/amortization_schedule/${this.conceptId}`]);
    }
    ...
    getLocationWiseCostBreakdown() {
        this._router.navigate([`rdpp-rtapp/location-wise-cost-breakdown/${this.conceptId}`]);
    }
    ...
    getConsultant() {
        this._router.navigate([`rdpp-rtapp/tpp-annexure-three/${this.conceptId}`]);

    }
    getProjectSummaryId(url){
        let routeUrl = "rdpp-rtapp/"+url+"/"+this.conceptId;
        this._router.navigate([routeUrl]);

    }

    getAnnexSix() {
        this._router.navigate([`/rdpp-rtapp/tpp-annexure-six/${this.conceptId}`]);
