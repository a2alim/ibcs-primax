...
export class TappPhysicalContingencyComponent extends UnsubscribeAdapterComponent implements OnInit {
    ...
    tappMasterId: number;
    conceptUuid: string;
    contingencyComponentInRtappFound: boolean = true;
    ...
    constructor(private formBuilder: FormBuilder,
        ...
        this.rtppAnnualPhasingCostService.fiscalYear.subscribe(res => {
            if (res) {
                this.addFiscalYear(res);
            }
        });
        rtppAnnualPhasingCostService.checkFiscalYear.subscribe(_ => {
            this.getContingencyDataInRtapp();
        });
    }
    ...
    private getContingencyDataInRtapp(): any {
        this.show = true;
        this.emptyAllArray();
        this.projectSummaryService.getByUuid(this.conceptUuid).subscribe(projectRes => {
            ...
            this.rtppAnnualPhasingCostService.getByProjectConceptIdAndComponentName({
                ...
                if (c) {
                    this.contingencyComponentInRtappFound = true;
                    ...
                    this.contingencyComponentInRtappFound = false;
                    this.getAllCall();
                }
            })
        });
    ...
    initFiscalYearWithCost(): any {
        this.rtppAnnualPhasingCostService.fiscalYearList.subscribe(res => {
    ...
    private arrangeData(next: boolean) {
        ...
        this.contingencyArray.forEach(e => {
            ...
            if (this.contingencyArray.length === i) {
                if (this.contingencyComponentInRtappFound) {
    ...
    private create(request: ITppAnnualPhasingCostWithChildDetailsRequest, next: boolean) {
        if(request.tappAnnualPhasingCostTabDetails[0].totalAmount == 0) {
                this.snackBar.openWarnSnackBarWithMessage("Require to Input Physical Contingency", OK);
                return;
        }
        this.spinner = true;
        this.rtppAnnualPhasingCostService.saveWithChild(request).subscribe(res => {
            if (res) {
    ...
    private update(request: ITppAnnualPhasingCostWithChildDetailsRequest, next: boolean) {
        this.spinner = true;
        this.rtppAnnualPhasingCostService.updateWithChild(request).subscribe(res => {
            if (res) {
                this.tappAnnualPhasingCostWithChildDetails = res;
                this.rtppAnnualPhasingCostService.setFiscalYearList(res.fiscalYearWiseCost.map(m => ({fiscalYear: m.fiscalYear})));
    ...
    reloadAll() {
        this.emptyAllArray();
        this.rtppAnnualPhasingCostService.setCheckFiscalYear();
    }
