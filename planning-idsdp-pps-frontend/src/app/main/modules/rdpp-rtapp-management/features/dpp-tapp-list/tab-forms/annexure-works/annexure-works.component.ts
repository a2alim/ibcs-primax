...
export class AnnexureWorksComponent implements OnInit {
    ...
    getRdppMasterData(masterId){
        this.rdppMasterService.getObjectiveCostByRdppMasterId(masterId).subscribe(res =>{
            ...
            console.log(this.rdppMasterData)
            this.getProjectSummary(res.projectConceptUuid);
        })
    }

    public getRdppAnnexureServiceData(): any {
        this.rdppAnnexureGoodsService.getDataList('get-list/Works/'+this.rdppMasterId).subscribe( res => {
            if(res.status == 0){
    ...
    getRevisedData(refId){
        this.rdppAnnexureGoodsService.getDataList('get-list/Works/'+ refId).subscribe( res => {
    ...
    delete(rowId, rowIndex) {
        this.spinner = true;
        this.rdppAnnexureGoodsService.deleteRow(rowId).subscribe((res) => {
            if(res.status == 1)
            {
    ...
    setFormControlData(response, type: string) {
