...
export class LocationWiseCostBreakdownComponent extends UnsubscribeAdapterComponent implements OnInit {
        ...
        this.subscribe$.add(
            this._rdppLocationService.getLocationByObjectiveCostIdUsingProjectSummary(this.rdppMasterId).subscribe(res => {
                ...
                if (res) {
                    ...
                    this.show = true;
                    this.snackbarHelper.openWarnSnackBarWithMessage("Require to save Project Location", OK);
                }
            }, _ => {
                this.saveDisable = true;
                this.show = true;
    ...
    keyUp($event: KeyboardEvent, index: number, field: 'quantity' | 'cost' | 'comment') {
        ...
        this.saveDisable = !this.upazilas.every(m => m.location.estimatedCost > 0 && m.location.quantity);
    }
