...
@Component({
    selector: 'app-procurement-plan-services',
    templateUrl: './procurement-plan-services.component.html',
    styleUrls: ['./procurement-plan-services.component.scss']
})
export class ProcurementPlanServicesComponent implements OnInit {
    ...
    saveData() {
        this.spinner = true;
        this.procurementPlanService.createProcurementPlan(this.form.value).subscribe((res: any) => {
    ...
    getProcurementPlan() {
        this.procurementPlanService.getGetProcurementPlanByDppTappMasterUuid('Service', this.dppTappMasterUuid).subscribe(
            response => {
    ...
    getProcurementPlanFromDppTapp() {
        this.annxGoodService.getDataList('get-list/Service/' + this.projectConceptUuid).subscribe(
            response => {
                if (response) {
    ...
    calculateCostAndQty() {
        ...
        this.form.controls['totalAmountView'].patchValue(totalValue_of_Goods.toFixed(2));
