...
import { locale as lngEnglishNaV } from '../../../../../../../layout/layouts/vertical/classy/i18n/en';
...
import { AttachmentModalListModel } from 'app/main/modules/dpp-tapp-management/models/attachmentModalList.model';
import { AttachmentUploadComponent } from 'app/main/shared/components/attachment-upload/attachment-upload.component';
...
import { GenerateReport } from "../../../../../../shared/model/generate-reports.model";
...
import {NotificationEventEnum} from "../../../../../../../layout/common/messages/notification-event.enum";
import {MessagesService} from "../../../../../../../layout/common/messages/messages.service";
import {WebsocketService} from "../../../../../../../layout/common/messages/websocket.service";
...
export class DppTappViewDashboardBetaComponent extends UnsubscribeAdapterComponent implements OnInit {
    ...
    showModal = false;
    message1 = "প্রকল্পটি অনুমোদিত হয়েছে";
    message2 = "ধন্যবাদ মাননীয় পরিকল্পনা মন্ত্রী";
    ...
    dataList: IYearWisePhysicalAndFinancialTarget[] = [];
    revenue: any = {};
    ...
    tappRevenueTotal: ITppPhasingCostTotal;
    ...
    bal: { componentName: string; tappAnnualPhasingCostTotal: ITppPhasingCostTotal[]; grandTotal: { fiscalYear: string; tappAnnualPhasingCostTotal: ITppPhasingCostTotal; }[]; };
    totalTappCost: any;
    ...
    ngOnInit(): void {
        ...
        this.conceptid = this.activatedRoute.snapshot.params['id'];
    ...
    getDppCurrentStage(dppMasterId) {
        this.stageMovementService.getCurrentStage(dppMasterId, 'DPP').subscribe(async res => {
            if (res.res) {
                this.projectStage = res.res.currentStage ? this.isEnLabel ? (res.res.currentStage).toString().split('_').join(' ') : this.statusStage.getProjectStatus(res.res.currentStage) : (this.isEnLabel ? 'IN AGENCY' : 'সংস্থায় আছে');
    ...
    getAllStageByMasterId(source, id) {
        this.stageMovementService.getAllStageByMasterId(source, id).subscribe(res => {
            ...
            this.movementStageList.forEach(e => {
                ...
                // e.currentStage = e.currentStage ? (this.isEnLabel ? e.currentStage.split('_').join(' ') : this.statusStage.getProjectStatus(e.currentStage)) : ('AGENCY DESK');
                ...
                // this.numberPipe.convertToBanglaNumber(time[1])
            });
    ...
    openUrl(row: any) {
        ...
        // row.module == 'pc' ? this.fileUploadService.download(row.attachment.urlPath) : this.fileUploadService.downloadAttachmentInDppService(row.attachment.urlPath);
    }

    deleteAttachment(row: any) {
        this.dashboardAttachmentService.deleteDashboardAttachmentById(row.uuid).subscribe(res => {
            this.spinner = false;
            if (res.value) {
                this.snackbarHelper.openSuccessSnackBarWithMessageEnBn('Attachment Successfully Deleted', 'সংযুক্তি সফলভাবে মুছে ফেলা হয়েছে');
                this.getListDashboardAttachment();
            } else {
                this.snackbarHelper.openErrorSnackBarWithMessageEnBn('Attachment is failed to delete', 'সংযুক্তি মুছে ফেলা করতে ব্যর্থ হয়েছে');
            }
        });
    }
    ...
    showComments(commission: 'A' | 'MD' | 'PC' | 'EC' | 'ECU' | 'PM', forward: string) {
        ...
        dialogRef.componentInstance.closeEventEmitter.subscribe(result => {
            if (result) {
                ...
                } else if (forward === 'returnToPlanning') {
                    this.forwardToPlanningMovement('returnToPlanning');
    ...
    getReportsList() {
        this.reportGenerateService.getReportList(this.projectSummary.uuid, this.projectSummary.projectTypeDTO.nameEn.toLowerCase()).subscribe(res => {
            this.reportList = res;
        });
    }
    compareReport() {
        ...
        if (len > 1) {
            const leftUrl = environment.ibcs.ppsReportEndPoint + this.reportList[len - 2].urlPath;
            const rightUrl = environment.ibcs.ppsReportEndPoint + this.reportList[len - 1].urlPath;
            let body = new FormData();
            body.append('identifier', '');
            body.append('left.display_name', 'Previous Report');
            body.append('left.file_type', 'pdf');
            body.append('left.source_url', leftUrl);
            body.append('right.display_name', 'Recent Report');
            body.append('right.file_type', 'pdf');
            body.append('right.source_url', rightUrl);
            body.append('public', 'true');
            this.reportGenerateService.sendFormDataForComparePdf(body).subscribe((response: any) => {
                if (response) {
                    const id = response.identifier;
                    const customizeUrl = '/dpp-tapp/compare-report/' + id;
                    window.open(customizeUrl, '_blank');
                }
            });
        } else {
            this.snackbarHelper.openErrorSnackBarWithMessage('Need at least two version to compare!', OK);
        }
    }
    ...
    sendDppTappToNothi() {
        ...
        dialogRef.componentInstance.closeEventEmitter.subscribe(res => {
            if (res) {
                this.sendNotifications(NotificationEventEnum.SEND_TO_E_NOTHI)
                dialogRef.close(true);
            }
        });
    }
        ...
        dialogRef.componentInstance.closeEventEmitter.subscribe(res => {
            if (res) {
                ...
                if (userGroupType === 'Agency') {
                    this.projectStatus = this.isEnLabel ? 'AGENCY DESK OFFICER ASSIGNED' : 'সংস্থার ডেস্ক অফিসার নিকট প্রেরণ করা হয়েছে';
                    ...
                    message = this.isEnLabel ? "Forwarded to ECNEC Officer successfully" : "সফলভাবে একনেক অফিসারের কাছে পাঠানো হয়েছে";
                    ...
                    this.projectStage = this.isEnLabel ? 'IN ECNEC' : 'একনেকে আছে';
                    message = this.isEnLabel ? "Forwarded to ECNEC Desk Officer successfully" : "সফলভাবে একনেক ডেস্ক অফিসারের কাছে পাঠানো হয়েছে";
                }
                this.sendNotifications(NotificationEventEnum.INTERNAL_MOVEMENT)
                //this.notificationComponent.getUsers(NotificationEventEnum.FORWARD_TO_MINISTRY_DESK);
    ...
    forwardToMinistryMovement() {
        ...
        this.openDialogProjectMovement((res) => {
            ...
            this.projectMovementService.forward(this.projectMovementModel).subscribe(res => {
                ...
                this.sendNotifications(NotificationEventEnum.PROJECT_FORWARD)
                if (this.isEnLabel)
    ...
    forwardToMinistry() {
        this.showComments("A", "forwardToMinistryHead");
    }
    ...
    toAgencyHeadMovement() {
        ...
        this.openDialogProjectMovement((res) => {
            ...
            this.projectMovementService.forward(this.projectMovementModel).subscribe(res => {
                ...
                this.projectStage = this.isEnLabel ? 'IN AGENCY' : 'সংস্থায় আছে';
                this.sendNotifications(NotificationEventEnum.PROJECT_RECAST)
                if (this.isEnLabel)
    ...
    returnToPlanningCommission() {
        this.showComments("PM", "returnToPlanning");
    }
    ...
    forwardToPlanningMovement(fromAction: string) {
        if (fromAction === 'unapprove') {
            this.forwardReturnAction = this.isEnLabel ? 'unapprove' : 'অননুমোদন করতে';
        } else if (fromAction === 'returnToPlanning') {
            this.forwardReturnAction = this.isEnLabel ? 'Return' : 'সংশ্লিষ্ট কমিশনে প্রেরণ করতে';
        } else {
            this.forwardReturnAction = this.isEnLabel ? 'Forward' : 'প্রেরণ করতে';
        }
        this.openDialogProjectMovement((res) => {
            ...
            this.projectMovementService.forward(this.projectMovementModel).subscribe(res => {
                ...
                this.currentStage = ProjectMovementStageConstant.PLANNING_COMMISSION_HEAD;
                if (fromAction === 'unapprove') {
                    this.sendNotifications(NotificationEventEnum.PROJECT_REJECTED)
                } else if (fromAction === 'forwardToPlanning') {
                    this.sendNotifications(NotificationEventEnum.PROJECT_FORWARD)
                }
                if (this.isEnLabel)
                    if (fromAction === 'returnToPlanning') {
                        this.snackbarHelper.openSuccessSnackBarWithMessage('Successfully send to Planning successfully', 'OK');
                    } else {
    ...
    forwardToPlanningMinister() {
        ...
        this.openDialogProjectMovement((res) => {
            ...
            this.projectMovementService.forward(this.projectMovementModel).subscribe(res => {
                ...
                this.isRelatedMeetingAttachments = false;
                this.projectStatus = this.isEnLabel ? 'PLANNING MINISTER' : 'পরিকল্পনা মন্ত্রী';
    ...
    approvedByMinistry() {
        ...
        this.openDialogProjectMovement((res) => {
            ...
            this.projectMovementService.forward(this.projectMovementModel).subscribe(res => {
                ...
                this.sendNotifications(NotificationEventEnum.PROJECT_APPROVED)
                if (this.isEnLabel)
    ...
    approvedByPlanningMinister() {
        ...
        this.openDialogProjectMovement((res) => {
            ...
            this.projectStatus = this.isEnLabel ? 'APPROVED' : 'অনুমোদিত';
            this.projectStage = this.isEnLabel ? 'APPROVED BY HONORABLE PLANNING MINISTER' : 'মাননীয় পরিকল্পনা মন্ত্রী দ্বারা অনুমোদিত';
            this.isInPlanningMinister = false;
            ...
            this.projectMovementService.forward(this.projectMovementModel).subscribe(res => {
                ...
                this.sendNotifications(NotificationEventEnum.PROJECT_APPROVED)
                // if (this.isEnLabel)
                //     this.snackbarHelper.openSuccessSnackBarWithMessage('The project has been Approved.\n' + 'Thank you Honorable Planning Minister.', 'OK');
                // else
                //     this.snackbarHelper.openSuccessSnackBarWithMessage('প্রকল্পটি অনুমোদিত হয়েছে ।\n' + 'ধন্যবাদ মাননীয় পরিকল্পনা মন্ত্রী ।', 'ওকে');
                // this.toggleModal();
                this.isDppProjectType ? this.getAllStageByMasterId('DPP', this.dppMasterId) : this.getAllStageByMasterId('TAPP', this.tappMasterId);
            });
        });
    ...
    submitMeetingNoticeWithAttachment() {
        ...
        if (meetingAttachmentType == 'Project Scrutiny Committee Notice') {
            ...
            this.isInDspecMeetingHeld = false;
        } else if (meetingAttachmentType == 'SPEC Meeting Notice') {
            this.projectMovementModel.currentStage = ProjectMovementStageConstant.SPEC_MEETING_NOTICE;
            projectStatus = this.isEnLabel ? 'Awaiting for SPEC Meeting' : this.statusStage.getProjectStatus(ProjectMovementStageConstant.SPEC_MEETING_NOTICE);
        } else if (meetingAttachmentType == 'SPEC Meeting Held') {
    ...
    savePotroJariAttachment() {
        ...
        dialogRef.componentInstance.closeEventEmitter.subscribe(res => {
            if (res) {
                ...
                this.canEdit = false;
                this.sendNotifications(NotificationEventEnum.ATTACH_POTROJARI)
            }
    ...
    submitConditionalEcnecApprove() {
        this.forwardReturnAction = this.isEnLabel ? 'Forward' : 'প্রেরণ করতে';
        this.currentStage = ProjectMovementStageConstant.ECNEC_CONDITIONAL_APPROVE;
        this.openDialogProjectMovement((res) => {
            if (res) {
                if (this.dppMasterId != null) {
                    this.projectMovementModel.dppMasterId = this.dppMasterId;
    ...
    saveConditionalEcnecApprove(ecnecConditionFormGroup) {
        this.projectMovementService.forward(this.projectMovementModel).subscribe(res => {
            ...
            this.fileUploadService.uploadConditionalEcnecAttachment(this.file, ecnecConditions).subscribe(res2 => {
                this.frmGroup.reset();
                this.snackbarHelper.openSuccessSnackBarWithMessageEnBn('Conditional approved done successfully', 'সফলভাবে শর্তসাপেক্ষে অনুমোদিত হয়েছে');
                this.isConditionalApproveByEcnec = false;
                this.isEcnecApprove = false;
                this.isUapproveByEcnec = false;
                this.projectStatus = this.isEnLabel ? "Conditionally Approved in ECNEC" : 'শর্তসাপেক্ষে অনুমোদন করা হয়েছে ';
                ...
                this.snackbarHelper.openErrorSnackBarWithMessageEnBn('Attachment is failed to save', 'সংযুক্তি সংরক্ষণ করতে ব্যর্থ হয়েছে');
            });
            this.isDppProjectType ? this.getAllStageByMasterId('DPP', this.dppMasterId) : this.getAllStageByMasterId('TAPP', this.tappMasterId);
        })
    }
    ...
    gotoPartATab(url: string) {
        const routeUrl = 'dpp-tapp/' + url + '/' + this.uuid;
    ...
    gotoSelectedTab(url: string) {
        ...
        } else if (url == 'dppFsReportBtn' || url == 'tappFsReportBtn') {

            setTimeout(() => {
                this.fsReportBtn.nativeElement.focus();
            }, 500);
        } else if (url == 'location-wise-cost-breakdown' || url == 'add-annexuregoods' || url == 'add-annexurewokrs' || url == 'add-annexureservices') {
            const routeUrl = 'dpp-tapp/' + url + '/' + this.uuid;
            this.route.navigate([routeUrl]);
        }
    }

    gotoFeasibilityButton(url: string) {
        if (url == 'dppFsReportBtn' || url == 'tappFsReportBtn') {
            setTimeout(() => {
                this.fsReportBtn.nativeElement.focus();
            }, 500);
        }
    ...
    downloadAnnexureIIIcReport() {
        ...
        this._reportDataService[methodName](this.uuid).subscribe(
            ...
            err => {
                this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
                console.log('downloadAnnexureIIIcReport : ', err);
                this.isLoading = false;
            }
        );
    }
    ...
    downloadAnnexureVbReport() {
        ...
        this._reportDataService[methodName](this.uuid).subscribe(
            ...
            err => {
                this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
                console.log('downloadAnnexureVbReport : ', err);
                this.isLoading = false;
            }
        );
    }
    ...
    downloadAnnexureSixReport() {
        ...
        this._reportDataService[methodName](this.uuid).subscribe(
            ...
            err => {
                this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
                console.log('downloadAnnexureSixReport : ', err);
                this.isLoading = false;
            }
        );
    }
    ...
    downloadPartAReport() {
        ...
        this._reportDataService[methodName](this.uuid).subscribe(
            ...
            err => {
                this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
                console.log('downloadPartAReport : ', err);
                this.isLoading = false;
            }
        );
    }
    ...
    downloadPartBReport() {
        ...
        this._reportDataService[methodName](this.uuid).subscribe(
            ...
            err => {
                this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
                console.log('downloadPartBReport : ', err);
                this.isLoading = false;
            }
        );
    }
    ...
    downloadFullDppReport() {
        ...
        this._reportDataService[methodName](this.uuid).subscribe(
            ...
            err => {
                this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");;
                console.log('downloadFullDppReport : ', err);
                this.isLoading = false;
            }
        );
    }
...
//Tapp Start
    ...
    //         err => {
    //             this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
    //             console.log('downloadTappAnexFiveReport : ', err);
    //             this.isLoading = false;
    //         }
    //     );
    // }
...
//Jasper
    ...
    workingSelectChange($event: MatSelectChange) {
        if ($event.value) {
            this.isLoading = true;
            this._reportDataService.getPscWorkingPlanReport(this.uuid, $event.value, this.userGroup.groupStatus).subscribe(
                res => {
                    this._reportCommonService.previewReport(res, "PDF");
                    this.isLoading = false;
                },
                err => {
                    this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
                    console.log('workingSelectChange : ', err);
                    this.isLoading = false;
                }
            );
        }
    ...
    summarySelectChange_Old($event: MatSelectChange) {
        if ($event.value) {
            if ($event.value == '') {
                ...
                this.projectSummariesService.getProjectSummaries(this.uuid, $event.value).subscribe(res => {
                    if (res.status == 0) {
                        ...
                        this._reportDataService.getSummaryReportByUrl(this.uuid, $event.value).subscribe(
                            ...
                            err => {
                                this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
                                console.log('summarySelectChange : ', err);
                                this.spinner = false;
                            }
                        );
                    }
    ...
    gotToEditDashboard() {
        this.route.navigate([`dpp-tapp/dashboard/${this.uuid}`]);
    }
    ...
    //     this.dashboardAttachmentService.getByPcIdAndProjectType(this.id, this.projectSummary.projectTypeDTO.nameEn).subscribe(dr => {
    //         dr?.forEach(e => e.module = 'pc');
    //         this.dashboardAttachmentService.getOtherAttachmentForPartBWithoutPageable(this.id).subscribe(or => {
    //             if (or) {
    ...
    uploadFsFile(file: FileList): any {
        this.file = file.item(0);
        this.saveAttachment(this.file);
    }

    saveAttachment(file: File): any {
        this.fileUploadService.uploadFileDppService(file).subscribe(res => {
            this.fsAttachmentId = res.id;
            this.fsAttachmentName = '';
            this.objectivesAndCost.fsAttachmentId = res.id;
            this.objectivesAndCost.fsAttachmentName = res.name;
            this.linkFsReportWithDpp();
        }, err => {
    ...
    openConditionalApproveByEcnecDialog() {
        ...
        dialog.afterClosed().subscribe(result => {
            if(result){
                this.sendNotifications(NotificationEventEnum.PROJECT_APPROVAL_WITH_CONDITION);
            }
    ...
    unapprovedByEcnec() {
        this.showComments("ECU", "unapprove");
    }
    ...
    private downloadTermOfRefReport() {
        ...
        this._reportDataService.getCommitteeReportByUrl(this.uuid, this.reportIndex).subscribe(res => {
            ...
            this.snackbarHelper.openWarnSnackBarWithMessageEnBn("Report Not Found !", "রিপোর্ট পাওয়া যায়নি !");
            console.log('downloadTermOfRefReport : ', error);
            this.spinner = false;
        });
    }

    createProjectSummaries() {
        let sectorName = (this.isEnLabel) ? this.sectorDivision?.sectorDivisionNameEn  :  this.sectorDivision?.sectorDivisionNameBn;
        localStorage.setItem('sectorName', sectorName);
        this.route.navigate(['dpp-tapp/project-summaries/' + this.projectSummary.uuid])
    }
    ...
    public sendProjectData() {
        ...
        this.dashboardService.sendProjectData(this.projectRequestModel).subscribe(res => {
            if (this.projectSummary.projectTypeDTO.nameEn == 'DPP') {
                window.open(environment.ibcs.gisUrl + 'projectInfo/' + this.dppMasterId + "?access_token=" + this.projectRequestModel.access_token, '_blank');
            } else if (this.projectSummary.projectTypeDTO.nameEn == 'TAPP') {
                window.open(environment.ibcs.gisUrl + 'projectInfo/' + this.tappMasterId + "?access_token=" + this.projectRequestModel.access_token, '_blank');
            }
            ...
            console.log('sendProjectData : ', error);
        });;
    }
    ...
    getProjectSummaryId(url, forUrl = "") {
        let routeUrl = '';
        if (forUrl == "") {
            routeUrl = "dpp-tapp/" + url + "/" + this.uuid;
        } else {
            routeUrl = url;
        }
        this.route.navigate([routeUrl]);
    ...
    saveMeetingNoticeWithAttachment(projectStatus) {
        this.projectMovementService.forward(this.projectMovementModel).subscribe(res => {
            ...
            this.fileUploadService.uploadApprovalProcessFlowAttachment(this.file, projectMovementStage).subscribe(res2 => {
                this.frmGroup.reset();
                this.snackbarHelper.openSuccessSnackBarWithMessageEnBn('Meeting saved successfully', 'মিটিং সফলভাবে সংরক্ষণ করা হয়েছে');
                this.isInProjectScrutinyCommitteeNotice = false;
                ...
                this.spinner = false;
                console.log('saveMeetingNoticeWithAttachment : ', err);
    ...
    saveWorkingPaperWithAttachment() {
        this.projectMovementService.saveWorkingPaperWithAttachment(this.file, this.currentMovementStage.id, this.setWorkingPaperType).subscribe(res => {
            if (res) {
                this.frmGroup.reset();
                this.snackbarHelper.openSuccessSnackBarWithMessageEnBn('Working Paper saved successfully', 'ওয়ার্কিং পেপার সফলভাবে সংরক্ষিত হয়েছে');
                this.dialog.closeAll();
                this.getProjectMovementAttachmentByDppUuid(this.currentMovementStage.id);
                this.getAllProjectMovementAttachment(this.currentMovementStage.id);
                this.ngOnInit();
            ...
            this.spinner = false;
            console.log('saveWorkingPaperWithAttachment : ', err);
            this.snackbarHelper.openErrorSnackBarWithMessage('Attachment is failed to save', 'OK');
        });
    }
    ...
    getUserProfileInfo(): any {
        this.userProfileService.getUserProfileInfo().subscribe(
            res => {
                if (res) {
            ...
            err => {
                console.log('getUserProfileInfo err : ', err);
            }
        );
    }
    ...
    //-----------------
        ...
        setTimeout(() => {
            if (this.projectSummary.isForeignAid) {
                this.download_Full_Tapp_Pdf_Report('Full Dpp Tapp', 'tapp/full_tapp_Report_En_22');
            } else {
                this.download_Full_Tapp_Pdf_Report('Full Dpp Tapp', 'tapp/full_tapp_Report_Bn_22');
            }
            this.isLoading = false;
    ...
    download_Full_Tapp_Pdf_Report($fileName = '', $templateName = '') {
        this.data['fileName'] = $fileName;
        this.data['templateName'] = 'pps-reports/' + $templateName;
        this.data['lng'] = (this.projectSummary.projectTypeDTO.nameEn.toUpperCase() == 'TAPP' && this.projectSummary.isForeignAid == false) ? 'bn' : 'en';
        // Part A
        this.data['agency'] = JSON.stringify(this.agencyModel);
        this.data['lng'] = this.projectSummary.isForeignAid ? 'en' : 'bn';
        this.data['projectSummary'] = JSON.stringify(this.projectSummary);
        ...
        this.subscribe$.add(
            this.projectSummaryService.getByUuid(this.conceptUuid).pipe(
                ...
                if (res.dapc) {
                    ...
                    this.revenueTotalDa = res.dapc.dppAnnualPhasingCostTotal;
                    console.log('revenueTotal', this.revenueTotal);
                    this.fiscalYearWiseCost = res.dapc.fiscalYearWiseCost;
                    console.log('fiscalYearWiseCost', this.fiscalYearWiseCost);
        ...
        this.subscribe$.add(
            this.projectSummaryService.getByUuid(this.conceptUuid).pipe(
                ...
                if (res.dapc) {
                    ...
                    this.capitalTotalDa = res.dapc.dppAnnualPhasingCostTotal;
                    this.fiscalYearWiseCapitalCost = res.dapc.fiscalYearWiseCost;
                    this.fiscalYearList = res.dapc.fiscalYearWiseCost.map(m => ({ fiscalYear: m.fiscalYear }));
                }
            })
    ...
    //     this.objectiveAndCostService.getAmortizationReport(this.conceptUuid).subscribe(res => {
    //         this.getAmortizationReportData = res;
    //     })
    // }
    ...
    sendNotifications(type) {
        let model:  any = {};
        model.type = type;
        model.agencyId = this.agencyModel?.id;
        model.ministryDivisionId = this.agencyModel?.ministryDivisionId;
        model.sectorDivisionId = this.sectorDivision?.id;
        this.userGroupService.getUsersForNotification(model).subscribe((res) => {
            if(res.length > 0) {
                // @ts-ignore
                let users:any = res;
                let receiver: any = [];
                users.forEach((user) =>{
                    ...
                    model.mobileNo = user.mobileNumber;
                    model.email = user.emailId;
                    model.username = user.name;
                    receiver.push(model);
                });
                console.log(receiver)
                let messageHubRequest: any ={};
                messageHubRequest.receiver = receiver;
                messageHubRequest.projectTitleEn = this.titleEn;
                messageHubRequest.projectTitleBn = this.titleBn;
                messageHubRequest.projectUuid = this.conceptid;
                messageHubRequest.senderId = this.userId;
                messageHubRequest.senderUserName = this.userName;
                messageHubRequest.eventType = type;
                // @ts-ignore
                this.websocketService._send(messageHubRequest);
            }

        });
    }

    toggleWorkingPaper = () => this.workingPaper = this.workingPaper = !this.workingPaper;
}
