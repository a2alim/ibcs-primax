...
export class PscWorkingPlanComponent extends UnsubscribeAdapterComponent implements OnInit {
    ...
    save() {
        this.form.patchValue({
            ...
            pscPaperType: this.paperType,
            userType: this.userGroup.groupStatus
        })
    ...
    private getPscWorkingPlan(paperType) {
        this.subscribe$.add(
            this.pscWorkingPlanService.getByPcId(this.conceptId, paperType, this.userGroup.groupStatus).subscribe(res => {
                    if (res) {
...
calDSpan(division, zilla, upazila, zi, ui){
    ...
    onChangePaperType($event): void {
        ...
        if ($event.value != 'select') {
            this.paperType = $event.value;
            this.pscPaperType = true;
            this.getPscWorkingPlan($event.value);
        } else {
            this.pscPaperType = false;
        }
    ...
    workingSelectChange() {
        this.isLoading = true;
        this._reportDataService.getPscWorkingPlanReport(this.conceptId, this.paperType, this.userGroup.groupStatus).subscribe(
            res => {
