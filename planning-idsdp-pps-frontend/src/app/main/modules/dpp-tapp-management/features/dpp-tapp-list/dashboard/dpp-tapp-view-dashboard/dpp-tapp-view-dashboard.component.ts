...
export class DppTappViewDashboardComponent implements OnInit {
    ...
    @ViewChild('callPlancommPresentation') callPlancommPresentationDialog: TemplateRef<any>;
    ...
    uploadpresentationFile(files: FileList) {
        this.presentation.presentationFile = files.item(0);
    }
    ...
    deskUserMovement(userGroupType) {alert(userGroupType);
        ...
        dialogConfig.data = {
            ...
            message: "Forwarded to Desk successfully",
    ...
    addPresentation(){
        ...
        const dialogRef = this.dialog.open(this.callPlancommPresentationDialog, {
            height: '480px',
            width: '800px',
            position: {
                top: '15vh',
        ...
        this.getPlancommPresentation();
    }

    savePlancommPresentation(){
        if(this.dppMasterId != null) {
            this.presentation.sourceId = this.dppMasterId;
            this.presentation.sourceModule='dpp';
        } else {
            this.presentation.sourceId = this.tappMasterId;
            this.presentation.sourceModule='tapp';
        }
        this.fileUploadService.uploadDppTappPresentation(this.presentation).subscribe(res=>{
            this.snackbarHelper.openSuccessSnackBarWithMessage('Presentation Added Successfully', 'OK');
            this.dialog.closeAll();
        },
        (error)=>{
            this.snackbarHelper.openErrorSnackBarWithMessage('Failed to add presentation', 'OK')
        });

    }

    getPlancommPresentation(){
        let sourceId,sourceModule;
        if(this.dppMasterId != null) {
            sourceId = this.dppMasterId;
            sourceModule ='dpp';
        } else {
            sourceId = this.tappMasterId;
            sourceModule='tapp';
        }
        this.dashboardService.getDppTappPresentation(sourceId, sourceModule).subscribe(res=>{
            this.dppTappPresentations = res.res;
        })
    }

    downloadPlancommPresentation(urlPath){
    ...
    checkCurrentStage(response: any) {
        ...
        if (this.currentStage === ProjectMovementStageConstant.AGENCY_DESK && (this.userGroup.groupStatus == 'AGENCY-DESK' || this.userGroup.groupStatus == 'OTHER')) {
            ...
            if((this.userGroup.groupStatus === 'PLANNING-HEAD' || this.userGroup.groupStatus === 'OTHER')){
                ...
                this.isAddPresentation = true;
            }

        }
        else if (this.currentStage == ProjectMovementStageConstant.PLANNING_COMMISSION_DESK ) {
            if(this.userGroup.groupStatus === 'PLANNING-HEAD' || this.userGroup.groupStatus === 'PLANNING-DESK' || this.userGroup.groupStatus === 'PLANNING-MINISTER' || this.userGroup.groupStatus === 'OTHER'){
                ...
                else if(this.userGroup.groupStatus === 'PLANNING-HEAD' || this.userGroup.groupStatus === 'PLANNING-MINISTER'){
                    this.isAddPresentation = true;
                }
            }
