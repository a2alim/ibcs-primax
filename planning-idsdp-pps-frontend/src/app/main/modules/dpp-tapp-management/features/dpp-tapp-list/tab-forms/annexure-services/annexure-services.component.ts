...
export class AnnexureServicesComponent implements OnInit {
    ...
    saveData() {
        this.spinner = true;
        this.annxGoodService.save(this.form.value).subscribe((res:any) => {
    ...
    setFormFieldValue() {
        this.annxGoodService.getDataList('get-list/Service/'+this.projectConceptUuid).subscribe((response) => {
            ...
            if (response.status > 0) {
                ...
                this.form.controls['totalAmountView'].setValue((!this.projectSummary.isForeignAid && this.projectSummary.projectTypeDTO.nameEn.toUpperCase() == 'DPP') ? this.numberPipe.convertToBanglaNumber(amount) : amount);;
