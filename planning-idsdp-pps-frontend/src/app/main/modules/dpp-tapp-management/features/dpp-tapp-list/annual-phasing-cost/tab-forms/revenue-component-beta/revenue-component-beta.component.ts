...
// initialize fiscal tear
    ...
    private arrangeData(next: boolean, exit: boolean) {
        if (this.revenueArray.some(s => !s.form.value.economicCodeId)) {
            this.snackBar.openWarnSnackBarWithMessage("Please give all economic code", OK);
        } else if (this.revenueArray.some(s => !s.form.value.subEconomicCodeId)) {
            this.snackBar.openWarnSnackBarWithMessage("Please give all sub economic code", OK);
        } else if (this.revenueArray.some(s => !s.form.value.unitId)) {
            this.snackBar.openWarnSnackBarWithMessage("Please give all unit", OK);
        } else if (this.revenueArray.some(s => s.details.qty < 0)) {
            this.snackBar.openWarnSnackBarWithMessage("Quantity can not be less than 0", OK);
        } else {
