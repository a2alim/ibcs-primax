export const locale = {
    lang: 'en',
    data: {
        homePage: 'Project Dashboard',
        /*----Page Header ----*/
        'page_title': 'Annexure - VI',
        'page_name': 'Amortization Schedule',
        'isShow': true,
        'heading': 'Annexure - VI Amortization Schedule',
        'title': 'Annexure - VI Amortization Schedule',
        'subHeading': 'Home ',
        'subHeading2': ' Annexure - VI Amortization Schedule',
        'sub_title': 'Home > Annexure - VI Amortization Schedule',
        'ProjectMainPage': 'Project Dashboard',
        /*----input field label ----*/
        'project_name' : 'Name of Project: ',
        'total_investment' : 'Total Investment:',
        'loan_portion' : 'Loan Portion:',
        'loan_period' : 'Loan Period:',
        'rate_of_interest' : 'Rate of Interest:',
        'grace_period' : 'Grace Period:',

        /*----table header ----*/
        'year' : 'Year',
        'SponsoringMinistry/Division' : 'Sponsoring Ministry/Division',
        'beginning_principal_amount' : 'Beginning Principal Amount',
        'yearly_fixed_amount' : 'Yearly fixed Amount to be paid (Principal)',
        'yearly_interest' : 'Yearly Interest to be paid',
        'total_payment' : 'Total Payment <br> (Capital + Interest)',
        'ending_principal_balance' : 'Ending Principal Balance',
        'validationPage' : 'There is no Loan From GOB Portion in Part-A, Amortization Schedule is not Applicable.',


        'nameofTheProject': 'Name of the Project  ',
        'sposoringMinistry': 'Sponsoring Ministry/ Division  ',
        'implimentingAgency': 'Implimenting Agency  ',

        'SaveNext' : 'Save & Next',
    }
};
