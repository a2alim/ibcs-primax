...
import {ERROR, FAILED_UPDATE, SUCCESSFULLY_SAVE, SUCCESSFULLY_SAVE_BN} from "../../../../../core/constants/message";
import {MatSelectChange} from "@angular/material/select";
import {MatDialog} from "@angular/material/dialog";
import {MatTableDataSource} from "@angular/material/table";
import {DppTappService} from "../../../services/dpp-tapp.service";
import {GreenPageModel} from "../../../models/green-page.model";
...
export class CreateNewDppComponent extends UnsubscribeAdapterComponent implements OnInit {
    ...
    @ViewChild('linkWithGreenPage') linkWithGreenPage: TemplateRef<any>;
    ...
    displayedColumns: string[] = ['sl', 'projectInfo', 'gob', 'action'];
    displayedColumnsValue: string[] = ['slValue', 'projectInfoValue', 'gobValue', 'actionValue'];
    paripatraVersionList: any [] = [];
    paripatroID: any;
    paripatroName: any;
    defultVirsion: any;
    ...
    ngOnInit(): void {
        ...
        this.getAllApi();
        this.getUserGroup();
        this.paripatraVersion();
        this.defultVirsion = this.paripatraVersionList[0].nameEn;
    }
    ...
    getUserInfoByUserId(userId) {
        this.userGroupService.geUserInfoByUserId(userId).subscribe(res => {
            ...
            this.agencyCode = res.agency.code;
            this.ministryCode = res.agency.ministryDivision.code;
        })
    }
    ...
    getAllProjectTitle() {
        this.service.getProjectByAgency().subscribe((res) => {
            this.projectTitleList = res;
    ...
    paripatraVersion(){
        this.service.paripatraVersion().subscribe(res => {
            this.paripatraVersionList = res;
            this.defultVirsion = this.paripatraVersionList[0].nameEn;
            res.forEach(paripatroId => {
                this.paripatroName = paripatroId.nameEn
            })
        })
    }
    onChangeparipatraVersion(value: any): void {
        if (value) {
            this.form.patchValue({
                paripatraVersionId: value
            });
        }
    }
    // Paripatra 2016
    ...
    private populateForm(): void {
        this.form = new FormGroup({
            ...
            paripatraVersionId: new FormControl(this.paripatraVersionList[0].id, [Validators.required]),
            ...
            pcLinkId: new FormControl(''),
            amsId: new FormControl(null),
            amsCode: new FormControl(null),
        });
        ...
        this.summaryForm = new FormGroup({
            ...
            isFeasibilityRequired: new FormControl('false', ),
    ...
    private populateWithSettingFormPatchValue() {
        this.form = new FormGroup({
            ...
            titleEn: new FormControl(this.projectSummary.titleEn, [Validators.required]),
            titleBn: new FormControl(this.projectSummary.titleBn, [Validators.required]),
            objectivesEn: new FormControl(this.projectSummary.objectivesEn),
            objectivesBn: new FormControl(this.projectSummary.objectivesBn),
            expCommencementDate: new FormControl(this.projectSummary.expCommencementDate, [Validators.required]),
            expCompletionDate: new FormControl(this.projectSummary.expCompletionDate, [Validators.required]),
            totalAmount: new FormControl(0),
            gobAmount: new FormControl(0),
            ...
            implementingAgencyName: new FormControl(this.projectSummary.implementingAgencyName),
            sourceModuleType: new FormControl('DPP_TAPP'),
            pcLinkId: new FormControl(this.projectSummary?.id),
        });
    ...
    setValue(res){
        this.form.patchValue({
            uuid: res.uuid,
            // paripatraVersionId: res.paripatraVersionId,
            projectCode: res.projectCode,
            projectTypeId: res.projectTypeId,
            priorityId: res.priorityId,
            titleEn: res.titleEn,
            titleBn: res.titleBn,
            objectivesEn: res.objectivesEn,
            objectivesBn: res.objectivesBn,
    ...
    openDialogGreenPage() {
        const dialogRef = this.dialog.open(this.linkWithGreenPage, {
            height: '650px',
            width: '1200px',
            position: {
                top: '1vh',
        ...
        dialogRef.afterClosed().subscribe(res => {
            // this.greenPageForm.reset();
        });

        this.dppTappService.getGreenPageList(this.ministryCode, this.agencyCode, "2023-24", 0).subscribe((res) => {
    ...
    selectGreenPage(row: GreenPageModel) {
        this.closeDialog();
        this.dppSave = true;
        this.dppSaveAndPcUpdate = false;
        this.setGreenPageProjectValue(row);
    }

    setGreenPageProjectValue(row){
        let sectorDivisionModel = this.sectorsDivisions.find(f => f.code === row.sector_Division_Code);
        this.getSectorDivision(sectorDivisionModel.id, row, sectorDivisionModel);
    }

    getSectorDivision(sectorDivisionId: number, row, sectorDivisionModel) {
        this.subscribe$.add(
            this.sectorService.getBySectorDivisionId(sectorDivisionId).subscribe(res => {
                this.sectors = res;
                let sectorModel = this.sectors.find(f => f.code === row.sector.keyword);
                this.getSector(sectorModel.id, row, sectorDivisionModel, sectorModel);
            })
        );
    }

    getSector(sectorId: number, row, sectorDivisionModel, sectorModel) {
        this.subscribe$.add(
            this.subSectorService.getBySectorId(sectorId).subscribe(res => {
                this.subSectors = res;
                let subSectorModel = this.subSectors.find(f => f.code === row.subsector.keyword);
                this.form.patchValue({
                    projectCode: row.project_title,
                    projectTypeId: row.projectTypeId,
                    priorityId: row.priority_id,
                    titleEn: row.project_title,
                    titleBn: row.project_tiltle_bn,
                    expCommencementDate: row.date_of_commencement,
                    expCompletionDate: row.date_of_completion,
                    sectorDivisionId: sectorDivisionModel.id,
                    sectorId: sectorModel.id,
                    subSectorId: subSectorModel?subSectorModel.id:null,
                    sponsoringMinistryName: row.ministry.ministry_name,
                    implementingAgencyName: row.agency.agency_name,
                    sourceModuleType: 'DPP_TAPP',
                    amsId: row._project_id,
                    amsCode: row.project_code,
                    isForeignAid: row.isOnlyGob=='No'?'true':'false'
                });
            })
        );
    }
